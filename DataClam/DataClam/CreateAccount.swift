...
func createPrivacyUser(userID: String)
{
    // Check if userID is saved on device
    
    
    // ----------
    
    // first check if user already exists by sending the public key
    ...
    // the parameters
    let postString = "userID=\(username.text!)"
